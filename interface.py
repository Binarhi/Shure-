...
def save_data():
    try:
        data_to_save = {
            "mics": mics,
            "X32_IP": entry_m32_ip.get(),
            "toggle_state": toggle_state
        }
        with open("Mics.txt", "w") as file:
...
def process_input():
    try:
        ...
        mic = {
            ...
            "receiver_ip": str(ip_address),
            "receiver_type": str(receiver_type),
            "receiver_ch": str(receiver_channel),
            "receiver_x32_ch": str(m32_channel),
            "M32 IP": m32_ip,
        }
...
entry_m32_ip.insert(0, "192.168.0.100")  # Set a default M32_IP value
...
button_toggle.grid(row=3, column=9, padx=10, pady=10)
